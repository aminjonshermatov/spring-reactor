...
public class ItemStreamsHandlerTest {
    ...
    public void setUp() {
        ...
        itemReactiveCappedRepository
                .insert(data())
                .doOnNext(itemCapped -> System.out.println("Inserted item in functional setup : " + itemCapped))
                .blockLast();
    }

    @Test
    public void testStreamAllItems() {
        Flux<ItemCapped> itemCappedFlux = webTestClient.get().uri(VERSION + ItemConstants.ITEM_STREAM_FUNCTIONAL_ENDPOINT)
                .exchange()
                .expectStatus().isOk()
